...
class AssessmentSlotOutputReport:
    """ Number of Test cases / use cases name """
    TestCases = 30
    use_case_name = 'ASSESSMENT SLOTS FLOW'
    ...
    def __init__(self, version, server, start_date_time):
        ...
        self.__path = outputFile.OUTPUT_PATH['AssessmentSlot_output']
        self.xlw = excelWrite.ExcelReportWrite(version=self.version, test_cases=self.TestCases)

        excel_headers = ['Captcha Page', 'Status', 'Slot Page', 'Status', 'Update Slot Page', 'Status']
        color_headers = ['Status', 'Captcha Page', 'Slot Page', 'Update Slot Page']
        ...
        self.__history_path = outputFile.OUTPUT_PATH['Assessment_slot_output_history']
        self.__html_path = outputFile.OUTPUT_PATH['Assessment_slot_output_html']
    ...
    def slot_page_report(self, entry_coll):
        testdata_headers = ['Select Slot Time', 'Save Slot', 'Thank you page']
        self.xlw.input_output_report(testdata_headers=testdata_headers, collection=entry_coll,
                                     row=2, i_column=2, o_column=3, path=self.__path)

    def update_slot_page_report(self, entry_coll):
        testdata_headers = ['Select Slot Time', 'Save Slot', 'Thank you page']
        self.xlw.input_output_report(testdata_headers=testdata_headers, collection=entry_coll,
